...
export async function POST(request: Request) {
  ...
  const trip = await prisma.trip.create({ data: { id: body.id, name: body.name, country: body.country, countryCode: body.countryCode || "🌍", city: body.city, startDate: new Date(`${body.startDate}T12:00:00`), endDate: new Date(`${body.endDate}T12:00:00`), people: Number(body.people) || 1, theme: body.theme || "blue", ownerId: user.id } });
