...
type CheckInRow = { completed: boolean };
...
function ensureCheckInSchema() {
  if (!checkInSchemaReady) {
    ...
    )`).then(() => undefined).catch((error) => { checkInSchemaReady = null; throw error; });
  }
...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const rows = await prisma.$queryRaw<CheckInRow[]>(Prisma.sql`SELECT "completed" FROM "TripCheckIn" WHERE "tripId" = ${id} AND "userId" = ${auth.user!.id}`);
  return NextResponse.json({ completed: rows[0]?.completed ?? false });
}
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (auth.response) return auth.response;
  await ensureCheckInSchema();
  const completed = Boolean((await request.json()).completed);
  const rows = await prisma.$queryRaw<CheckInRow[]>(Prisma.sql`
