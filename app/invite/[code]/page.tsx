...
export default function InvitePage() { const { code } = useParams<{ code: string }>(); const router = useRouter(); const [invite, setInvite] = useState<Invite | null>(null); const [error, setError] = useState(""); const [loading, setLoading] = useState(true);
  ...
  async function accept() { if (!invite?.user) { router.push(`/auth?next=${encodeURIComponent(`/invite/${code}`)}`); return; } setLoading(true); const response = await fetch(`/api/invites/${code}/accept`, { method: "POST" }); const result = await response.json(); setLoading(false); if (!response.ok) { setError(result.error); return; } router.push(`/trips/${result.tripId}/overview`); }
