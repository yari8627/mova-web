...
import { CalendarDays, CheckCircle2, ChevronRight, Clock3, FileCheck2, Luggage, MapPin, Paperclip, ReceiptText, Settings, TicketCheck, Users, WalletCards } from "lucide-react";
...
type Document = { id: string; name?: string; offline: boolean; bookingId?: string | null; storageKey?: string | null };
...
const starterDocuments: Document[] = [{ id: "passport", offline: true }, { id: "insurance", offline: true }];
const starterExpenses: Expense[] = [{ id: "hotel", amount: 420 }, { id: "dinner", amount: 135.6 }, { id: "train", amount: 276 }];
const starterParticipants: Participant[] = [{ id: "giulia", status: "confirmed" }, { id: "marco", status: "confirmed" }, { id: "luca", status: "confirmed" }];
const money = new Intl.NumberFormat("it-IT", { style: "currency", currency: "EUR" });

function readSaved<T>(key: string, fallback: T): T { const saved = window.localStorage.getItem(key); return saved ? JSON.parse(saved) as T : fallback; }
function tripGroup(remote: { owner?: { id: string; name: string; email: string } | null; participants: Participant[] }) { const owner = remote.owner ? { id: `owner-${remote.owner.id}`, status: "confirmed" as const } : null; return [owner, ...remote.participants].filter(Boolean) as Participant[]; }

export default function OverviewPage() {
  const { id } = useParams<{ id: string }>(); const router = useRouter();
  const { canInvite } = useTripPermissions(id);
  const [activities, setActivities] = useState<Activity[]>([]); const [bookings, setBookings] = useState<Booking[]>([]); const [documents, setDocuments] = useState<Document[]>([]); const [expenses, setExpenses] = useState<Expense[]>([]); const [participants, setParticipants] = useState<Participant[]>([]); const [budget, setBudget] = useState<number | null>(null); const [tripDates, setTripDates] = useState<TripDates | null>(null); const [packingItems, setPackingItems] = useState<PackingItem[]>([]); const [checkInCompleted, setCheckInCompleted] = useState(false); const [savingCheckIn, setSavingCheckIn] = useState(false);
  ...
  useEffect(() => { Promise.all(["personal", "shared"].map((scope) => fetch(`/api/trips/${id}/packing?scope=${scope}`, { cache: "no-store" }).then((response) => response.ok ? response.json() as Promise<PackingItem[]> : []))).then(([personal, shared]) => setPackingItems([...personal, ...shared])).catch(() => setPackingItems([])); }, [id]);
  useEffect(() => { fetch(`/api/trips/${id}/check-in`, { cache: "no-store" }).then((response) => response.ok ? response.json() : null).then((value) => { if (value) setCheckInCompleted(Boolean(value.completed)); }).catch(() => undefined); }, [id]);
  ...
  const today = useMemo(() => {
    ...
    if (current < start) return { phase: "before" as const, day: 0, distance: Math.ceil((start.getTime() - current.getTime()) / 86400000), activities: [] as Activity[] };
    if (current > end) return { phase: "after" as const, day: daysFromStart + 1, distance: Math.floor((current.getTime() - end.getTime()) / 86400000), activities: [] as Activity[] };
  ...
  async function updateCheckIn(completed: boolean) {
    ...
    try {
      const response = await fetch(`/api/trips/${id}/check-in`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ completed }) });
  ...
  return <main className="trip-detail-shell overview-shell">
    ...
    <section className="overview-grid">
      <button className="overview-card overview-next" onClick={() => router.push(`/trips/${id}`)}><span className="overview-icon"><CalendarDays size={22} /></span><div><small>PROSSIMA ATTIVITÀ</small>{nextActivity ? <><strong>{nextActivity.title}</strong><p><Clock3 size={15} /> Giorno {nextActivity.day} · {nextActivity.time} · {nextActivity.place}</p></> : <><strong>Programma completato</strong><p><CheckCircle2 size={15} /> Non ci sono attività da svolgere</p></>}</div><ChevronRight size={20} /></button>
      <button className="overview-card" onClick={() => router.push(`/trips/${id}/bookings`)}><span className="overview-icon"><TicketCheck size={22} /></span><div><small>PRENOTAZIONI</small><strong>{confirmedBookings} confermate</strong><p>{pendingBookings ? `${pendingBookings} in attesa` : "Nessuna in attesa"}</p></div><ChevronRight size={20} /></button>
