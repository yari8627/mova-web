...
import { ArrowLeft, Check, Clock3, Copy, Mail, Pencil, Plus, Trash2, UserRound, Users, X } from "lucide-react";
...
export default function ParticipantsPage() {
  ...
  async function save() { if (!draft.name.trim() || !draft.email.trim()) return; if (editingId) { persist(people.map((person) => person.id === editingId ? { ...person, ...draft } : person)); setShowEditor(false); return; } const response = await fetch(`/api/trips/${id}/invites`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(draft) }); const result = await response.json(); if (!response.ok) return; const next = [...people.filter((person) => person.email.toLowerCase() !== result.participant.email.toLowerCase()), result.participant as Participant]; setPeople(next); announce(next); setInviteCode(result.code); setInviteLink(`${window.location.origin}${result.link}`); setShowEditor(false); }
  ...
  return <main className="trip-detail-shell participants-shell">
    ...
    <div className="participants-grid">
      ...
      <aside className="invite-panel"><div className="invite-icon"><Users size={27} /></div><h2>Invita al viaggio</h2><p>{inviteCode ? "Condividi il link con la persona appena invitata." : "Crea un invito personale indicando nome, email e ruolo."}</p>{inviteCode ? <><div className="invite-code"><strong>{inviteCode}</strong><button onClick={copyCode} aria-label="Copia link invito"><Copy size={18} /></button></div>{copied && <span className="copy-feedback"><Check size={14} /> Link copiato</span>}<a className="secondary-button invite-email" href={`mailto:?subject=${encodeURIComponent("Invito al viaggio MOVA")}&body=${encodeURIComponent(`Unisciti al viaggio su MOVA: ${inviteLink}`)}`}><Mail size={18} /> Invita tramite email</a></> : <button className="secondary-button invite-email" onClick={openNew}><Plus size={18} /> Crea invito</button>}</aside>
    </div>
    {showEditor && <div className="modal-backdrop" onMouseDown={() => setShowEditor(false)}><div className="modal participant-modal" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}><div className="modal-header"><div><p className="section-kicker">PARTECIPANTE</p><h2>{editingId ? "Modifica persona" : "Nuovo invito"}</h2></div><button className="icon-button" onClick={() => setShowEditor(false)} aria-label="Chiudi"><X size={20} /></button></div><form className="trip-form" onSubmit={(event) => { event.preventDefault(); save(); }}><label>Nome<input value={draft.name} onChange={(event) => setDraft({ ...draft, name: event.target.value })} placeholder="Nome del partecipante" required autoFocus /></label><label>Email<input type="email" value={draft.email} onChange={(event) => setDraft({ ...draft, email: event.target.value })} placeholder="nome@email.com" required /></label><label>Ruolo<select value={draft.role} onChange={(event) => setDraft({ ...draft, role: event.target.value as Participant["role"] })}><option value="participant">Partecipante</option><option value="co-organizer">Co-organizzatore</option></select></label>{!editingId && <div className="invite-status-note"><Clock3 size={18} /><span>Il partecipante resterà “In attesa” finché non accetterà l’invito.</span></div>}<div className="modal-actions"><button type="button" className="secondary-button" onClick={() => setShowEditor(false)}>Annulla</button><button type="submit" className="primary-button"><UserRound size={18} /> {editingId ? "Salva" : "Invia invito"}</button></div></form></div></div>}
  </main>;
}
